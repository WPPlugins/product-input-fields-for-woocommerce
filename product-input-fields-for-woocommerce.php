...
if ( ! defined( 'ALG_WC_PIF_VERSION' ) ) {
	define( 'ALG_WC_PIF_VERSION', '1.1.0' );
}
...
final class Alg_WC_PIF {
	...
	function includes() {
		...
		$settings[] = require_once( 'includes/admin/class-alg-wc-pif-settings-general.php' );
		$settings[] = require_once( 'includes/admin/class-alg-wc-pif-settings-per-product.php' );
		$settings[] = require_once( 'includes/admin/class-alg-wc-pif-settings-all-products.php' );
		if ( 'yes' === get_wc_pif_option( 'global_enabled', 'yes' ) ) {
			for ( $i = 1; $i <= apply_filters( 'alg_wc_product_input_fields', 1, 'all_products_total_fields' ); $i++ ) {
				$settings[] = new Alg_WC_PIF_Settings_All_Products_Field( $i );
			}
		}
